...
class TextMelLoader_refine(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, metadata_file_path, hparams, is_inference=False):
        ...
        if hparams.use_f0 and not is_inference:
            self.sf_dict={}
            self.uv_dict={}

            self.sf = np.load(self.mel_dir + 'sf.npz')
            print('unzip sf to sf_dict')
            for i in range(len(self.sf.files)):
                self.sf_dict[self.sf.files[i]]=self.sf[self.sf.files[i]]
            print('unzip sf to sf_dict has finished!')

            self.uv = np.load(self.mel_dir + 'uv.npz')
            print('unzip uv to uv_dict')
            for i in range(len(self.uv.files)):
                self.uv_dict[self.uv.files[i]]=self.uv[self.uv.files[i]]
            print('unzip uv to uv_dict has finished!') 
        ...
        for i in range(len(self._metadata)):
            ...
            self.utt_data[str(i)] = {
                                    "text":torch.IntTensor(self.text_to_sequence(item.strip().split('|')[5] , self.text_cleaners)),
                                    "mel":torch.from_numpy(np.load(self.mel_dir + 'out_' + item.strip().split('|')[0] + '.npy'))*8.0-4.0,#(outputs[outputs.files[i]]+4.0)/8.0 in prepare_refine_data.py
                                    ...
                                    "sf":self.sf_dict[item.strip().split('|')[0]] if hparams.use_f0 and not is_inference else None,
                                    "uv":self.uv_dict[item.strip().split('|')[0]] if hparams.use_f0 and not is_inference else None,
                                    }
    ...
    def get_mel_text_pair(self, meta_data):
        ...
        if self.hparams.use_f0:
            ret_pair.append(get_f0(meta_data["sf"], meta_data["uv"], dur))
