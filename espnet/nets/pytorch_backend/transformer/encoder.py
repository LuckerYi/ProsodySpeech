...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.normalize_before:
            self.after_norm = LayerNorm(attention_dim, hparams=self.hparams)
    ...
    def forward(self, xs, masks, spembs_=None):
        ...
        if isinstance(
            ...
            xs = self.embed(xs)
        xs, masks, spembs_= self.encoders(xs, masks, spembs_)
        if self.normalize_before:
            xs = self.after_norm(xs, spembs_)
        return xs, masks
