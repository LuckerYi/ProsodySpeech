...
class Postnet(torch.nn.Module):
    ...
    def __init__(
        ...
        for layer in six.moves.range(n_layers - 1):
            ...
            if use_batch_norm:
                ...
                self.layer_norms += [
                        LayerNorm(ochans,hparams=hparams,dim=1,elementwise_affine=True)
                ]
        ...
        if use_batch_norm:
            ...
            self.layer_norms += [
                        LayerNorm(odim,hparams=hparams,dim=1,elementwise_affine=True)
                ]
    ...
    def forward(self, xs):
        ...
        if self.use_batch_norm:
            ...
            for i in six.moves.range(len(self.postnet) - 1):
                xs = self.postnet[i](xs)            
                xs = self.dropout(self.tanh(self.layer_norms[i](xs)))
            xs = self.postnet[-1](xs)
            xs = self.dropout(self.layer_norms[-1](xs))
        return xs
