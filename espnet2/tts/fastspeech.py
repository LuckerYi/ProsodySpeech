...
class FastSpeechLoss(torch.nn.Module):
    ...
    def forward(self, after_outs, before_outs, d_outs, ys, ds, ilens, olens, p_outs=None, ps=None):
        ...
        Args:
            ...
            ps (Tensor): Batch of pitchs (B, Tmax).
        ...
        if self.use_masking:
            ...
            pitch_masks = duration_masks if p_outs is not None else None
...
class FastSpeech(AbsTTS):
    ...
    def __init__(
        ...
        if self.hparams.use_f0:
            ...
            self.pitch_predictor_embed = torch.nn.Linear(1, adim)   
    ...
    def _forward(
        ...
        if is_inference:
            p_outs = self.pitch_predictor.inference(hs, d_masks, spembs_=spembs_, is_f0=True) if self.hparams.use_f0 else None # (B, Tmax) or None
            if self.hparams.use_f0:
                hs = hs + torch.tanh(self.pitch_predictor_embed(p_outs.unsqueeze(-1))) # (B, Tmax, adim)
        else:
            if self.hparams.stop_gradient_from_pitch_predictor:
                p_outs = self.pitch_predictor(hs.detach(), d_masks, spembs_=spembs_) if self.hparams.use_f0 else None # (B, Tmax) or None
            else:
                p_outs = self.pitch_predictor(hs, d_masks, spembs_=spembs_) if self.hparams.use_f0 else None # (B, Tmax) or None
            if self.hparams.use_f0:
                hs = hs + (torch.tanh(self.pitch_predictor_embed(ps.unsqueeze(-1))) if not self.hparams.f0_joint_train else torch.tanh(self.pitch_predictor_embed(torch.clamp(p_outs.exp() - self.pitch_predictor.offset, min=0).unsqueeze(-1))))# (B, Tmax, adim)
        ...
        if self.use_gst:
            if self.hparams.is_partial_refine:
                ...
                if self.hparams.use_mutual_information and is_MI_step:
                    # return hs.detach(), style_embs.unsqueeze(1).expand(-1, hs.size(1), -1).detach()
                    return hs_random_choosed.detach(), style_embs.detach()
                ...
                if self.hparams.style_embed_integration_type == "concat":
                    ...
                    hs = hs + (style_embs.unsqueeze(1) if self.hparams.gst_train_att else style_embs)# hyper parameters gst_train_att means is there second attention?
        ...
        if is_inference:
            ...
            sigma = self.sigma_predictor(torch.cat([hs,ds.float().unsqueeze(-1)],dim=-1), d_masks, spembs_=spembs_) if self.hparams.use_gaussian_upsampling else None # (B, Tmax)
    ...
    def forward(
        ...
        l1_loss, duration_loss, l2_loss, pitch_loss = self.criterion(
            after_outs, before_outs, d_outs, ys, ds, ilens, olens, p_outs, pitch
        )
        ...
        if self.hparams.use_f0:
            pitch_loss = pitch_loss*self.hparams.f0_alpha
            loss += pitch_loss
