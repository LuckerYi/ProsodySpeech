...
class ReferenceEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if hparams.gst_reference_encoder == 'convs':
            ...
            layer_norms = torch.nn.ModuleList()
            ...
            for i in range(conv_layers):
                ...
                layer_norms += [LayerNorm(conv_out_chans,hparams=hparams,elementwise_affine=True)]
            self.convs = convs
    ...
    def forward(self, speech: torch.Tensor, spembs_: torch.Tensor = None, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.hparams.gst_reference_encoder == 'convs':
            ...
            for i in range(len(self.convs)):
                xs = self.convs[i](xs) # (B, conv_out_chans, Lmax', idim')
                xs = self.layer_norms[i](xs.transpose(1,3))# (B, Lmax', idim', conv_out_chans)
                xs = self.relu(xs.transpose(1,3).transpose(2,3))# (B, conv_out_chans, Lmax', idim')
            hs = xs.transpose(1, 2)# (B, Lmax', conv_out_chans, idim')
